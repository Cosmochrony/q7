...
\usepackage{hyperref}
\hypersetup{
  pdftitle={Toward Three Spatial Directions from a Supplied Spin-One Carrier: Requirements for an
    Equivariant Bridge},
...
\begin{document}
  ...
  \begin{abstract}
    ...
    three whenever the two coefficients are positive; invariance under the Cartan generator $J_3$ separates the spin-weight $\pm 1$
  ...
  \subsection{The Admissible Three: The Neutral Sector of a Supplied Spinor Carrier}
    ...
    resolved modes filling $\Heff$ at that numerical threshold~\cite{Beau2026a32}; O29~\cite{Beau2026a33} reports that this rank is modal rather
...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    \begin{remark}[Stage~B anomaly at $q=211$]
      ...
      $90^\circ$ to within $4 \times 10^{-10}$ degrees at all twelve pairs, consistent with
      $F_c^4 = I$.
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    \paragraph{The ordering.}
      ...
      off-block entries of $C_c$ itself, relative to its largest eigenvalue, are at most
      $1.4 \times 10^{-15}$, a figure the diagnostic prints per pair.
    ...
    \paragraph{What happens at $(151,5)$.}
      ...
      the covariance plane is degenerate to within $5.6 \times 10^{-16}$, the figure the diagnostic
      prints for the gap inside the plane at that pair.
  ...
  \subsection{Reproducible diagnostic}
    ...
    spectrum of the compression; and a warning distinguishing the pairs where the plane is
