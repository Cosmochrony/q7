...
\begin{document}
  ...
  \subsection{Rigidity of Any Bridge: Schur's Lemma}
    ...
    \begin{remark}[Consequence for the bridge]
      \label{rem:rigidity}
      Lemma~\ref{lem:schur} implies that, under Hypothesis~[ID] and [ACT], \emph{there is at most
      one equivariant bridge up to one scalar} between $\Heff$ and a three-dimensional adjoint
      module.
...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{The Bridge Conjecture}
    ...
    \begin{conjecture}[Bridge existence]
      ...
      three-dimensional real spatial sector of the effective co-metric, carrying the $\su(2)$-action
      [ACT] that makes it an adjoint module, compatible with the Schr\"{o}dinger representation in
      the following sense.
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    O29 pools the projections over all conjugate pairs at a given prime, whereas the protocol here
    keeps them separate, one covariance per pair.
    Per pair the normalised spectrum is $[1, 0.672, 0.666]$ at $(61,3)$, $[1, 0.920, 0.920]$ at
    $(61,5)$ and $[1, 0.500, 0.500]$ at $(61,7)$; pooled over all the conjugate pairs stored at that
    prime it is $[1, 0.7527, 0.7513]$, which is O29's $[1:0.8:0.8]$, and the same pooling at $q = 29$
    gives $[1, 0.5584, 0.5584]$, which is O29's $[1:0.6:0.6]$.
    Both of O29's published spectra are therefore reproduced from the inputs used here, and the
    per-pair figures are not in disagreement with them: they are a finer aggregation of the same
    measurement. Nothing in this paper depends on either.
    The pooled figures are the one numerical statement in this section that the shipped bundle
    cannot reproduce, since it stores one covariance per pair and the pooling is not recoverable
    from those; \texttt{--pooled} in the diagnostic of Section~\ref{ssec:diagnostic} computes them
    from the checkpoints, and prints the pair and projection counts with each.
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    \paragraph{The column label.}
      ...
      $0.0574$ at $(151,5)$.
      Read as an isotropy ratio on that rotated matrix the formula would give $0.2772$, and the
      script's own isotropy variable, which divides by $A_H$ rather than by
      $|\tilde L_{11}|$, gives $0.3218$.
      Four readings, three distinct values, and the published column is the one its running text
  ...
  \subsection{Reproducible diagnostic}
    ...
    The \texttt{--pooled} flag instead reports the covariance pooled over all conjugate pairs at
    each prime, the aggregation O29 uses; that path needs the checkpoints, for the reason given in
