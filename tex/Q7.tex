...
\begin{document}
  ...
  \begin{abstract}
    ...
    under an equivariant bridge is positive definite of rank three, as a Hermitian form, whenever
    the two coefficients are positive; invariance under the Cartan generator $J_3$ separates the spin-weight $\pm 1$
  ...
  \section{Introduction and Statement of the Problem}
    ...
    \subsection{Context}
      ...
      \paragraph{The admissible ``3'' (O-series).}
        ...
        input), then its traceless anti-Hermitian sector is
  ...
  \subsection{The Admissible Three: The Neutral Sector of a Supplied Spinor Carrier}
    ...
    Paper O23~\cite{Beau2026a27} proves that if the admissible neutral sector is carried by an
    irreducible two-dimensional unitary representation $V_\rho \cong \mathbb{C}^2$ (a supplied
    input), then its traceless anti-Hermitian sector is $\su(2) \simeq \ImH$, carrying exactly
    three independent
  ...
  \subsection{The Schr\"odinger Quadratic Form Lemma}
    ...
    \begin{lemma}[Schr\"{o}dinger quadratic form]
      ...
      \begin{enumerate}
        ...
        \emph{Hermitian} (sesquilinear) extension to $W_\mathbb{C}$ of the real spatial block
        ...
        and $\lambda_H = A_H / |\lambda|^2$, $\lambda_z = A_z / |\lambda|^2$ with $\lambda$ the
        scalar of Lemma~\ref{lem:schur}, positive under the same proviso.
    ...
    \begin{proof}
      ...
      For items~(2) and~(3): the spatial form $h_{\mathrm{sp}}$, Hermitian on $W_\mathbb{C}$ and
      equal to $\mathrm{diag}(A_H, A_H, A_z)$
      ...
      The pullback $\varphi^* h_{\mathrm{sp}}$ therefore acts by $A_H/|\lambda|^2$ on
      $\{e_+, e_-\}$ and by $A_z/|\lambda|^2$ on $\{e_0\}$, giving~(3).
...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    This $3 \times 3$ matrix is not the object O28 calls $C_c$, which is a $9 \times 9$ covariance
    of vectorised outer products in $\mathrm{End}(\Heff)$; the two carry the same name in the
    corpus and are kept apart here.
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    \paragraph{The rotation inside the horizontal plane, and when it is harmless.}
      ...
      Measured in the plane, with eigenvalues normalised by the largest of $C_c$, the gap lies
      between $10^{-16}$ and $6 \times 10^{-16}$ at ten pairs, $(101,3)$ included, and is resolved
      at two: $6.2 \times 10^{-3}$ at $(61,3)$ and $1.6 \times 10^{-2}$ at $(101,7)$.
    ...
    \paragraph{The column label.}
      ...
      $|L_{11} - L_{22}|/|L_{11}|$, an expression it did not say which matrix to evaluate on.
      On the stored-basis compression $\tilde L$ it is $0$, both entries being equal; on the
      reported $L_{\mathrm{sw}}$ at $(151,5)$ it is $0.2772$, whereas the quantity actually
      printed is the cross-term ratio
  ...
  \subsection{What This Paper Establishes}
    ...
    \begin{itemize}
      ...
      equivariant bridge the pullback of the Hermitian spatial form is graded as in
      Lemma~\ref{lem:qform}, with the premises listed in Remark~\ref{rem:status}. Invariance under the Cartan generator forces isotropy
