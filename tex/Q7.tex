...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    The two non-flat stored modes $e_{\pm k}$ give equal diagonal entries
    by~\eqref{eq:rayleigh}, so the covariance eigenbasis $U$ of Stage~A is determined only up to a
    unitary rotation inside their span, and the reported matrix $L_{\mathrm{sw}}$ depends on which
    rotation the eigensolver returns.
    At ten of the twelve pairs $U$ acts as a permutation and $L_{\mathrm{sw}} = \tilde L$ up to
    reordering. Two pairs behave differently, and they are precisely the two pairs of
    Table~\ref{tab:numerical} whose stored indices differ by $\pm c \bmod q$.
    ...
    At $(q,c) = (151,5)$ the rotation is genuine. In the stored basis
    ...
    inside a degenerate eigenspace, not a measured anisotropy; the invariant content is the pair of
    eigenvalues, given by~\eqref{eq:rayleigh}.
