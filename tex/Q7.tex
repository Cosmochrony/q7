...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    Taken over all thirty, the median of the per-pair spectra is $[1, 0.736, 0.732]$, and over the
    ...
    $[1:0.6:0.6]$.
    O29 does not state its aggregation for these two figures in its text, but it ships the routine:
    the \texttt{pooled} function of its \texttt{variety\_summary.py}, which concatenates the
    trajectories over the pairs of a prime, returns these same two spectra when applied to this
    covariance.
    ...
    The pooled and median figures are the statements of this section that the shipped bundle cannot
    reproduce, and the obstacle is coverage rather than form: the pooled covariance is the mean of
    the per-pair covariances weighted by their projection counts, both of which the bundle carries,
    but the bundle holds three of the thirty pairs at $q = 61$ and nothing at $q = 29$.
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    \paragraph{The column label.}
      ...
      $0.0574$ at $(151,5)$.
      Read as an isotropy ratio on that rotated matrix, with $|(L_{\mathrm{sw}})_{11}| = 6.9527$ in
      the denominator as the formula prescribes, it would give $0.2772$; the script's own isotropy
      variable divides the same numerator by the mean $A_H$ instead and gives $0.3218$.
  ...
  \subsection{Reproducible diagnostic}
    ...
    The \texttt{--pooled} flag instead reports, per prime, the covariance pooled over the conjugate
    pairs that hold projections and the median of their per-pair spectra, the aggregation that
    matches O29's published figures; that path needs the checkpoints, for the reason given in
