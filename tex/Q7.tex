...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    elsewhere it reports three trailing singular values as ``[c]omputed on the pooled Q5a-O5
    checkpoints with the summary script'', and that script, \texttt{variety\_summary.py}, has a
    \texttt{pooled} function which concatenates the trajectories over the pairs of a prime.
    Forming $\langle w w^\dagger \rangle$ on its output gives the two spectra above.
    That last step is this paper's, not O29's: no routine in O29's code forms this $3 \times 3$
    matrix, its covariances being those of vectorised outer products, on
    $\mathrm{End}(\Heff)$ and on a two-dimensional truncation of it.
