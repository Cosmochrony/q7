...
\title{Toward Three Spatial Directions from a Supplied Spin-One Carrier:\\
...
\begin{document}
  ...
  \begin{abstract}
    ...
    compared.
    Third, the $\su(2)$ Casimir acts on $\Sym^2(\Vr)$ as $2 \cdot \mathrm{Id}$, and its pullback
    under an equivariant bridge is positive definite of rank three whenever the two coefficients
    are positive; invariance under the Cartan generator $J_3$ separates the spin-weight $\pm 1$
  ...
  \section{Introduction and Statement of the Problem}
    ...
    \subsection{Context}
      ...
      \paragraph{The admissible ``3'' (O-series).}
        ...
        consistent with the selection rule $\Sigma_c(n_3) = 3$, and reports rank
        $r_{\mathrm{eff}} = 3$ for the per-pair covariance, stating that this is a finite-data
        value of its protocol and not an invariant; alongside it O28 reports a participation-ratio
        value $r_{\mathrm{eff}}^{\mathrm{PR}} = 8/3$.
        ...
        quotient $\mathcal{Q}$ (Theorem~4.1(i)), without equivariance; the identification
        $H_{\mathrm{eff}} \simeq \su(2)$ holds only under O23's supplied carrier together with
        O27's hypothesis of admissible saturation (Theorem~4.1(ii)), which O27 states is assumed
        and derived nowhere.
  ...
  \subsection{The Problem}
    ...
    hypothesis of Section~\ref{sec:sym2} it is unique up to one scalar by irreducibility
    (Lemma~\ref{lem:schur}), and it must satisfy a specific quadratic-symbol constraint
  ...
  \subsection{Conventions}
    ...
    $\mathrm{Im}\,\Pi_{\mathrm{adm}} \cap \mathrm{Ntrl}$, a \emph{real} three-dimensional space;
    ...
    O27 states that no dimension count transfers between the two in either direction, and O23
    warns against writing $\su(2) \cong \Sym^2(\Vr)$ without distinguishing a real from a complex
    dimension.
    ...
    $H_{\mathrm{eff}}$ with a subscript spelled out whenever the O27 object is meant.
  ...
  \subsection{The Admissible Three: The Neutral Sector of a Supplied Spinor Carrier}
    ...
    directions (Theorem~3.1); O23's Proposition~3.2 notes that a count of generator axes can stop
    ...
    $\Heff$ completely~\cite{Beau2026a32}; O29~\cite{Beau2026a33} reports that this rank is modal
    rather than invariant across pairs and primes, and that it is a rank in
    $\mathrm{End}(\Heff)$, not in $\mathrm{End}(\Vr)$.
    ...
    The ``admissible~3'' is the complex dimension of $\Heff$ (equivalently, the modal rank
    of $C_c$).
    ...
    \begin{remark}[Three distinct ``3''s to be kept separate]
      ...
      (iii) $\dim_\mathbb{C}(\Heff) = 3$ with modal covariance rank $r_{\mathrm{eff}} = 3$
  ...
  \subsection{The Symmetric Square Identification}
    ...
    On the carrier data supplied by O23~\cite{Beau2026a27} together with O27's saturation
    hypothesis~\cite{Beau2026a31}, and with the minimality selection $d_\rho = 2$ of
    O26~\cite{Beau2026a30}, we have $\Vr \simeq \mathbb{C}^2$ as an $\su(2)$-module: the unique
    irreducible spin-$\tfrac{1}{2}$ representation.
  ...
  \subsection{Rigidity of Any Bridge: Schur's Lemma}
    ...
    \begin{remark}[Consequence for the bridge]
      ...
      A \emph{positive real} normalisation is not a consequence of any of this, and it is not
      delivered by comparing forms either: demanding that $\varphi$ carry a chosen
      positive-definite Hermitian form on $\Sym^2(\Vr)$ to a positive-definite form on
      $W \otimes \mathbb{C}$ constrains only the modulus $|\lambda|$ of the scalar, since a phase
      $e^{i\theta}$ acts trivially on $\varphi^* h(v,v) = |\lambda|^2 h(v,v)$.
  ...
  \subsection{The Schr\"odinger Quadratic Form Lemma}
    ...
    \begin{lemma}[Schr\"{o}dinger quadratic form]
      \label{lem:qform}
      Let $\varphi : \Sym^2(\Vr) \xrightarrow{\sim} W_{\mathrm{sp}} \otimes_\mathbb{R} \mathbb{C}$
      be any $\su(2)$-equivariant isomorphism onto the complexified spatial sector (the unique such
      map up to one scalar, by Lemma~\ref{lem:schur}; the complexification is required, the source
      being a complex three-dimensional module and $W_{\mathrm{sp}}$ a real three-dimensional
      space).
      Then:
      \begin{enumerate}
        ...
        $\varphi^* g_{\mathrm{sp}}$ (where $g_{\mathrm{sp}} = \mathrm{diag}(A_H, A_H, A_z)$ is the
        spatial block of $g^{\mu\nu}$, extended $\mathbb{C}$-bilinearly) is positive-definite of
        rank~$3$.
        ...
        and $\lambda_H = A_H / \|\varphi\|^2$, $\lambda_z = A_z / \|\varphi\|^2$, positive under the
        same proviso.
      \end{enumerate}
    \end{lemma}
    ...
    \begin{proof}
      ...
      The $\mathrm{U}(1)$ argument is independent of the existence of the full bridge $\varphi$,
      but it requires the admissible regime to be $J_3$-invariant, which is a hypothesis
      of~\cite{Beau2026q5b} and not established here.
...
Item~(3) and the full form of~(2) are conditional on four further premises: the existence of the
...
with [H-L] not established there; the $J_3$-invariance of the admissible regime; and the
positivity of the two coefficients, of which $A_H > 0$ is stipulated inside [H-L] and $A_z$ is
...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{The Bridge Conjecture}
    ...
    \begin{conjecture}[Bridge existence]
      ...
      three-dimensional real spatial sector of the effective co-metric, compatible with the
      Schr\"{o}dinger representation in the following sense.
    ...
    conditional.
    Its source side is Hypothesis~[ID]; its target side exists only under [H-L], which
    \cite{Beau2026q5b} declares not established, with the Lorentzian reading carrying the further
    hyperbolicity hypothesis of its Theorem~6.1.
  ...
  Lemma~\ref{lem:schur} removes the ambiguity about \emph{what} to test: under Hypothesis~[ID] a
  non-zero equivariant bridge is unique up to one scalar, so a single criterion suffices.
  ...
  \subsection{Metaplectic symmetry of the discrete Weil Laplacian}
    ...
    corrected here. Both relations are verified numerically to machine precision by the diagnostic
    of Section~\ref{ssec:diagnostic}, and the commutation below is unaffected, since
    $L_{\mathrm{Weil}}$ contains each generator together with its adjoint.)
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    We compute compressions of $L_{\mathrm{Weil}}$ using the Q5a-O5 checkpoints produced
    ...
    eigenbasis.
    That second step matters: $U$ is not a permutation, and where the two non-flat modes give equal
    entries it can rotate freely inside their eigenspace (Section~\ref{ssec:rotation}).
    ...
    \begin{table}[h]
      ...
      \caption{What the compression $\tilde L = B_{\mathrm{eff}} L_{\mathrm{Weil}}
        ...
        the largest discrepancy over all rows and pairs is $3.6 \times 10^{-15}$.
    ...
    to the bridge. What the pipeline reports is $L_{\mathrm{sw}}$, which coincides with $\tilde L$
    at ten of the twelve pairs and differs at the two discussed in
    Section~\ref{ssec:rotation}.
    ...
    $\rho_{Z|XY} = 0.485$; at $(151,5)$ the modes $\pm 73$ differ by $146 \equiv -5$, giving the
    reported $\rho_{XY} = 0.057$.
    ...
    \begin{remark}[Universal value $A_z = 2$ and the $\mathfrak{su}(2)$ Casimir]
      \label{rem:az2}
      The entry $\tilde L_{00}$ equals $2$ exactly, not approximately, in every pair whose Stage~A
      ordering puts the flat mode first, that is in eleven of the twelve pairs; at
      $(q,c) = (101,3)$ the ordering puts the mode $k = 5$ first, and the entry $2$ appears in the
      other diagonal slot.
    ...
    \begin{remark}[Stage~B anomaly at $q=211$]
      ...
      Two features of $\tilde F$ are exact on these data and follow from
      Section~\ref{ssec:rayleigh}: since $F_c$ maps a Fourier mode to a vector supported at a
      single position, every entry of $\tilde F$ has modulus exactly $q^{-1/2}$, so $\tilde F$ is
      $q^{-1/2}$ times a unimodular matrix; and its eigenvalue phases are exact multiples of
      $90^\circ$, consistent with $F_c^4 = I$, at all twelve pairs.
    ...
    decreases with it; for $c = 3$ it does not, $k/q$ being essentially constant near $0.047$.
    ...
    \begin{remark}[Three-case structure of the isotropy question]
      ...
      (Papers Q8, Q9 and Q10 cite this structure as ``Q7 Remark~6.8'': Q9 restates all three
      cases, Q8 and Q10 name Case~3 and Case~1 respectively. In the deposited version~1.3 and here
      it is Remark~\ref{rem:threecases}, Corollary~\ref{cor:quasithm} being 6.8. The numbering of
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    The freedom in $U$ is a property of the covariance, not of the Rayleigh quotients: equality of
    the two non-flat entries of $\tilde L$ says nothing about $C_c$.
    What matters is the spectrum of $C_c$ itself.
    Measured on the twelve pairs, with eigenvalues normalised by the largest, the gap between the
    second and third eigenvalues is at machine precision, between $10^{-16}$ and
    $2 \times 10^{-15}$, at nine pairs, and is resolved at three: $6.2 \times 10^{-3}$ at
    $(61,3)$, $1.2 \times 10^{-2}$ at $(101,3)$ and $1.6 \times 10^{-2}$ at $(101,7)$.
    At the nine exactly degenerate pairs the eigenvectors spanning that plane are fixed only up to
    a unitary rotation of the plane, so $L_{\mathrm{sw}}$ there is not determined by the data: it
    depends on what the eigensolver returns.
    In particular the eight of those nine pairs that report a vanishing off-diagonal entry are not
    ``unaffected'' in any intrinsic sense; a different solver, ordering or seed could distribute
    the same pair of eigenvalues differently across the two slots and produce an off-diagonal term.
    The invariant content at a degenerate pair is the multiset of eigenvalues of
    $L_{\mathrm{sw}}$ restricted to that plane, which is $\{4 - 2\cos(2\pi k/q)\}$ with
    multiplicity two by~\eqref{eq:rayleigh}, together with the flat-mode eigenvalue $2$; the
    individual diagonal entries and the off-diagonal entry are not invariants.
    At the three resolved pairs $U$ is determined up to phases and acts as a permutation.

    At $(q,c) = (101,3)$ the reordering puts the mode $k = 5$ first, which is the origin of the
    published row $A_z = 2.096$, $A_H = \tfrac12(2.096 + 2.000) = 2.048$.

    At $(q,c) = (151,5)$, one of the nine exactly degenerate pairs (gap $1.5 \times 10^{-16}$), the
    rotation returned by the solver is not aligned with the stored modes. In the stored basis
    $\tilde L = \mathrm{diag}(2,\, 5.989188,\, 5.989188)$ with one off-diagonal entry of modulus
    $1$; after conjugation, $L_{\mathrm{sw}} = \mathrm{diag}(2,\, 6.9527,\, 5.0257)$ with an
    off-diagonal entry of modulus $0.2677$ between the two non-flat slots.
    The value $A_H = 5.989188$ survives only as the mean of the two split entries, by preservation
    of the trace, and the ratio $0.2677/\bar\lambda = 0.0574$ with $\bar\lambda = 4.6595$ is the
    number printed as $\rho_{XY} = 0.057$ in version~1.3.
    That column was captioned as the XY \emph{isotropy} ratio
    $|\tilde L_{11} - \tilde L_{22}|/|\tilde L_{11}|$, which is a different quantity: the isotropy
    ratio at this pair is $0.3217$ and the cross-term ratio is the $0.0574$ reported.
    The caption is corrected here.
    So the split and the off-diagonal entry at $(151,5)$ are an artefact of an arbitrary rotation
    inside an exactly degenerate covariance eigenplane, not a measured anisotropy; the invariant
    content is the pair of eigenvalues, given by~\eqref{eq:rayleigh}.
    The same artefact is available at the other eight degenerate pairs and simply did not occur in
    this run.

    One published number is not accounted for.
    The value $\rho_{XY} = 0.005$ printed at $(101,3)$ in version~1.3 is reproduced by no quantity
    the versioned script computes: at that pair the isotropy ratio is $0.0469$ and the cross-term
    ratios vanish.
    We record this rather than reconstruct it: the number's provenance is unknown, and no statement
  ...
  \subsection{Reproducible diagnostic}
    ...
    checkpoints instead, which are not redistributed here.
    ...
    It also prints the normalised spectrum of $C_c$ with the gap between its second and third
    eigenvalues, the matrix after conjugation by the covariance eigenbasis, and a warning at the
    pairs where that plane is degenerate and the individual entries are therefore solver-dependent
    (Section~\ref{ssec:rotation}).
    On the four primes and three characters reported here the lowest purity is $1.000000$ and the
    largest discrepancy between measurement and closed form is $3.6 \times 10^{-15}$; every
  ...
  \subsection{What This Paper Establishes}
    ...
    \begin{itemize}
      ...
      \item The $\su(2)$ Casimir acts on $\Sym^2(\Vr)$ as $2\cdot\mathrm{Id}$, and under an
      equivariant bridge its pullback is graded as in Lemma~\ref{lem:qform}, with the premises
      listed in Remark~\ref{rem:status}. Invariance under the Cartan generator forces isotropy
      ...
      by the covariance eigenbasis. That basis is fixed by the data only where the covariance
      spectrum is non-degenerate, which on these pairs means three of twelve; at the nine
      degenerate pairs the individual diagonal and off-diagonal entries are solver-dependent and
      only the eigenvalues are invariant (Section~\ref{ssec:rotation}). The closed form of
      Section~\ref{ssec:rayleigh} therefore describes the stored-basis compression exactly, and the
      published entries only up to that rotation.
    \end{itemize}
  ...
  \subsection{If the Identification Succeeds}
    ...
    carrier selection is an open bridge), on O27's saturation hypothesis~\cite{Beau2026a31}, on
    O26's minimality selection~\cite{Beau2026a30}, and on the existence of the effective operator,
    which requires [H-L] of~\cite{Beau2026q5b}.
    That is a chain of four supplied premises, and it should be presented as such wherever the
    result is used.
  ...
  and its pullback of the Casimir form is graded, isotropic within the horizontal plane, and equal
  to the spatial block up to the two constants $A_H$ and $A_z$, neither of which is evaluated here.
