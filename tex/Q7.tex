...
\begin{document}
  ...
  \begin{abstract}
    ...
    and paper O28 measures an admissible projection space $\Heff = \mathbb{C}^3$ whose per-pair
    covariance has rank three.
    ...
    under an equivariant bridge is positive definite of rank three; invariance under the Cartan
    generator $J_3$ separates the spin-weight $\pm 1$ sector from the spin-weight $0$ sector and
  ...
  \subsection{The Geometric Three: Carnot Grading and Spatial Directions}
    ...
    \begin{itemize}
      ...
      \item the central slot is \emph{degenerate at this order}: no coefficient $A_z$ is supplied
      by~\cite{Beau2026q5b}, whose Theorem~5.2 puts $0$ there and defers a full-rank spatial block
      to sub-principal corrections (open problem Q5b-O2).
    \end{itemize}
    ...
    Throughout this paper, $A_z$ denotes the \emph{hypothetical} coefficient of $k_Z^2$ that a
    full-rank spatial block would carry; no source supplies its value.
  ...
  \subsection{Rigidity of Any Bridge: Schur's Lemma}
    ...
    \begin{remark}[Consequence for the bridge]
      ...
      A \emph{positive real} normalisation is a further requirement, not a consequence: it holds
      once one demands that $\varphi$ carry a chosen positive-definite Hermitian form on
      $\Sym^2(\Vr)$ to a positive-definite form on $W \otimes \mathbb{C}$, which fixes
      $|\varphi|$ and leaves a phase.
      Version~1.3 stated uniqueness ``up to positive scalar'' without that requirement.
    \end{remark}
  ...
  \subsection{The Schr\"odinger Quadratic Form Lemma}
    ...
    \begin{proof}
      ...
      of~\cite{Beau2026q5b}, and no source supplies $A_z$ at all.
    \end{proof}
...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{The Bridge Conjecture}
    ...
    finitely many Fourier modes is the zero form, a bounded multiplication form with no
    differential symbol of positive order, or $+\infty$ (Q5a Theorem~6.1); in particular no such
    limit supplies a spatial second-order operator.
  ...
  \begin{criterion}[Symbol restriction to $\Sym^2(\Vr)$]
    ...
    directions with the real weight vectors of $\Sym^2(\Vr)$: the horizontal pair
    $(k_X, k_Y)$ with the real and imaginary parts of the weight-$\pm 1$ sector, that is with
    $\tfrac{1}{\sqrt 2}(e_+ + e_-)$ and $\tfrac{i}{\sqrt 2}(e_+ - e_-)$, and the central direction
    $k_Z$ with $e_0$.
  ...
  \begin{remark}[What is computable, and what Section~\ref{sec:numerical} computes]
    ...
    which concern the attributions this paper made, not the editorial status of those papers.
  \end{remark}
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    and the XY ratio $\rho_{XY} = |\tilde{L}_{11} - \tilde{L}_{22}|/|\tilde{L}_{11}|$,
    where $\bar\lambda$ is the mean diagonal entry.
    The quantities historically named $A_z := \tilde L_{00}$ and
    $A_H := \tfrac12(\tilde L_{11} + \tilde L_{22})$ are therefore diagonal entries of a
    compression, in the basis that Stage~A orders, of an operator on $\mathbb{C}^q$.
    ...
    \begin{table}[h]
      ...
      \caption{What the compression $\tilde L = B_{\mathrm{eff}} L_{\mathrm{Weil}}
        ...
        exactly when an off-diagonal entry of modulus $1$ appears.
      ...
      \begin{tabular}{rrrccccc}
        ...
        \hline
        61 &  3 &   3 & 2.000000 & 2.094729 & 2.094729 & 0.094729 & yes ($0 - (-3) = c$) \\
    ...
    \noindent
    Every entry of Table~\ref{tab:numerical} is thus accounted for by~\eqref{eq:rayleigh} and by
    the index-difference rule, with no free parameter and no appeal to the bridge.
    ...
    \begin{remark}[The cross term at $(q,c) = (61,3)$ is an index coincidence]
      ...
      The stored indices there are $\{0, -3, +3\}$ and the character is $c = 3$, so two of them
      differ by exactly $c$; by the index-difference rule of Section~\ref{ssec:rayleigh} the
      corresponding off-diagonal entry has modulus $1$, and dividing by the mean diagonal entry
      $\bar\lambda \approx 2.06$ returns the reported value.
    ...
    \begin{remark}[Stage~B anomaly at $q=211$]
      ...
      The Stage~B failure is expected rather than anomalous: the stored rows are Fourier modes, and
      $F_c$ maps a Fourier mode to a vector concentrated at a single position, so the overlaps
      measured by Stage~B are of order $q^{-1/2}$ for every $q$ and every $c$, whatever the
      coverage.
      In particular Stage~B does not test a property of the trajectory, and its threshold
      $0.75$ cannot be met by a basis of this form.
      Version~1.3 attributed the failure to low coverage, by analogy with the $(61,3)$ anomaly,
      and described the Stage~C outcome as analytically grounded via
      Corollary~\ref{cor:blockdiag}; both readings are withdrawn.
      Stage~C returns, here as elsewhere, the closed-form entries of
    ...
    \begin{remark}[Three-case structure of the isotropy question]
      ...
      (Papers Q9 and Q10 cite this three-case structure as ``Q7 Remark~6.8''; in the deposited
      version~1.3 and here it is Remark~\ref{rem:threecases}, Corollary~\ref{cor:quasithm} being
      6.8. The numbering of this paper is unchanged in version~2.0.)
    \end{remark}

  \subsection{Reproducible diagnostic}
    \label{ssec:diagnostic}

    The statements of Section~\ref{ssec:rayleigh} are checked by the script
    \texttt{code/q7\_mode\_diagnostic.py} of this repository, which reads the O25 checkpoints
    \texttt{q\{61,101,151,211\}\_o25.npz} of the Q5a-O5 campaign and prints, for each stored pair:
    ...
    off-diagonal modulus matches the index-difference rule, being $1$ at $(61,3)$ and $(151,5)$ and
    $0$ elsewhere.
  ...
  \subsection{What This Paper Establishes}
    ...
    \begin{itemize}
      ...
      \item Every entry of the compressions computed on the O25 checkpoints is the Rayleigh
      quotient~\eqref{eq:rayleigh} of $L_{\mathrm{Weil}}$ on a single Fourier mode, with an
      off-diagonal entry exactly when two stored indices differ by $\pm c \bmod q$
      (Section~\ref{ssec:rayleigh}, diagnostic in Section~\ref{ssec:diagnostic}). In particular the
      difference between the two distinct entries is the identity~\eqref{eq:gap}.
    \end{itemize}
  ...
  \subsection{Claims No Longer Asserted in Version 2.0}
    ...
    \paragraph{False statements about sources.}
      ...
      [H-L], no $A_z$ is supplied, and its Theorem~3.2 is a Gromov--Hausdorff statement carrying no
      rate.
