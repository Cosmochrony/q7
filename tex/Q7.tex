...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{Reproducible diagnostic}
    ...
    It also prints the normalised spectrum of $C_c$, the gap inside the horizontal plane, the matrix
    after conjugation by the covariance eigenbasis, the spectrum of the compression, and a warning
    distinguishing the pairs where that plane is degenerate and the block non-scalar, so that the
    individual entries are solver-dependent, from those where the block is scalar and they are not
    (Section~\ref{ssec:rotation}).
