...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{The Bridge Conjecture}
    ...
    hyperbolicity hypothesis of its Theorem~6.1, and carries the adjoint $\su(2)$-action [ACT] only
    by hypothesis, no source supplying it.
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    $(61,5)$ and $[1, 0.500, 0.500]$ at $(61,7)$, and the per-pair median is $[1, 0.736, 0.732]$ at
    $q = 61$ and $[1, 0.512, 0.512]$ at $q = 29$, neither of which rounds to a figure O29 publishes.
    ...
    $[1:0.6:0.6]$.
    O29 does not state its aggregation for these two figures, so the inference here is only that
    pooling is the one aggregation of this object that lands on both of them; the agreement is at
    O29's one-decimal precision, and the pooled spectrum at $q = 61$ is not exactly degenerate,
    whereas O29's remark records $\lambda_2 = \lambda_3$.
    ...
    from those; \texttt{--pooled --primes 29 61} in the diagnostic of
    Section~\ref{ssec:diagnostic} computes them from the checkpoints, and prints with each the
    number of stored pairs, the number that hold projections and the number of projections pooled.
    Those counts differ at $q = 29$, where $5$ of the $14$ stored pairs hold any.
