...
\begin{document}
  ...
  \begin{abstract}
    ...
    compared.
    Third, the $\su(2)$ Casimir acts on $\Sym^2(\Vr)$ as $2 \cdot \mathrm{Id}$, and an equivariant
    bridge pulls the Hermitian spatial form back to a positive-definite form of rank three whenever
  ...
  \subsection{The Admissible Three: The Neutral Sector of a Supplied Spinor Carrier}
    ...
    paper also calls $C_c$, has threshold rank $r_{\mathrm{eff}}^{1\%} = 3$, filling $\Heff$
    completely~\cite{Beau2026a32}; O29~\cite{Beau2026a33} reports that this rank is modal rather
  ...
  \subsection{The Schr\"odinger Quadratic Form Lemma}
    ...
    spatial form pulls back, under any equivariant bridge, to a positive-definite graded form on
    $\Sym^2(\Vr)$.
...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    \paragraph{The ordering.}
      ...
      $\{\text{flat}\} \oplus \{e_{\pm k}\}$ at all twelve pairs, to numerical precision (relative
      off-block entries at most $1.4 \times 10^{-15}$), so the flat mode is not mixed into the
      others; what can change is the slot it occupies.
    ...
    \paragraph{The rotation inside the horizontal plane, and when it is harmless.}
      ...
      Measured in the plane, with eigenvalues normalised by the largest of $C_c$, the gap is at
      most $2.1 \times 10^{-15}$ at ten pairs, $(101,3)$ included, one of them returning exactly
      zero, and it is resolved at two: $6.2 \times 10^{-3}$ at $(61,3)$ and
      $1.6 \times 10^{-2}$ at $(101,7)$.
    ...
    \paragraph{What happens at $(151,5)$.}
      ...
      the covariance plane is degenerate to within $6.1 \times 10^{-16}$, measured inside the plane
      as the previous paragraph requires.
  ...
  \subsection{Reproducible diagnostic}
    ...
    third eigenvalues and the one inside the horizontal plane, and keys its verdict to the second;
    then the matrix after conjugation by the covariance eigenbasis, the spectrum of the
    compression, and a warning distinguishing the pairs where the plane is degenerate and the block
    non-scalar, so that the individual entries are solver-dependent, from those where the block is
    scalar and they are not (Section~\ref{ssec:rotation}).
  ...
  \subsection{What This Paper Establishes}
    ...
    \begin{itemize}
      ...
      Lemma~\ref{lem:qform}, with the premises listed in Remark~\ref{rem:status}. Invariance under the Cartan
      ...
      scalar, that is at eleven of the twelve pairs. One pair, $(151,5)$, has both an arbitrary rotation and a non-
      scalar block, and it is
  ...
  and the Hermitian spatial form pulls back to a graded form on the module, isotropic within the
  horizontal plane and determined by the two constants $A_H$ and $A_z$, neither of which is
  evaluated here.
