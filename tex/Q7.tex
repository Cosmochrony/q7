...
\begin{document}
  ...
  \subsection{The Admissible Three: The Neutral Sector of a Supplied Spinor Carrier}
    ...
    directions (Theorem~3.1); O23's Proposition~3.4 notes that a count of generator axes can stop
...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    \paragraph{The ordering.}
      ...
      $\{\text{flat}\} \oplus \{e_{\pm k}\}$ at all twelve pairs, so the flat mode is never mixed
      into the others; what can change is the slot it occupies.
      ...
      $A_z = 2.096$, $A_H = \tfrac12(2.096 + 2.000) = 2.048$.
      That is a resolved gap of about one per cent, not a degeneracy.
    ...
    \paragraph{The rotation inside the horizontal plane, and when it is harmless.}
      ...
      Measured, with eigenvalues normalised by the largest, the gap between the second and third
      is at machine precision, between $10^{-16}$ and $2 \times 10^{-15}$, at nine pairs, and is
      resolved at two: $6.2 \times 10^{-3}$ at $(61,3)$ and $1.6 \times 10^{-2}$ at $(101,7)$
      (at $(101,3)$ the degenerate plane is the same one, the resolved gap being the flat mode's).
      At the nine degenerate pairs the rotation of that plane is arbitrary.
      It is nevertheless without effect at eight of them, for a reason worth stating: there the
      ...
      Exactly one pair has both an arbitrary rotation and a non-scalar block, and it is
      $(q,c) = (151,5)$.
    ...
    \paragraph{What happens at $(151,5)$.}
      ...
      the covariance plane is degenerate to $1.5 \times 10^{-16}$.
    ...
    \paragraph{The column label.}
      Version~1.3 captioned that column as the XY isotropy ratio
      $|\tilde L_{11} - \tilde L_{22}|/|\tilde L_{11}|$, which at $(151,5)$ evaluates to
      $0.2772$, whereas the quantity actually printed is the cross-term ratio
      $|(L_{\mathrm{sw}})_{12}|/\bar\lambda = 0.0574$, and the isotropy ratio as the script
      computes it, with $A_H$ in the denominator, is $0.3218$.
      Three different numbers; the caption named neither of the two the script produces.
  ...
  \subsection{What This Paper Establishes}
    ...
    \begin{itemize}
      ...
      arbitrary at the nine pairs where the covariance is degenerate but has no effect wherever the
      restriction of the compression to that plane is scalar, that is at eleven of the twelve
      pairs. One pair, $(151,5)$, has both an arbitrary rotation and a non-scalar block, and it is
      the only pair whose reported entries are solver-dependent.
