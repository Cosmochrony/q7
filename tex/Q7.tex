...
constants $A_H$ and $A_z$, which the lemma does not evaluate.
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    Nor is it the linear Hermitian covariance whose spectrum O29 reports: on these checkpoints the
    $3 \times 3$ matrix used here has normalised spectrum $[1, 0.672, 0.666]$ at $(61,3)$,
    $[1, 0.920, 0.920]$ at $(61,5)$ and $[1, 0.500, 0.500]$ at $(61,7)$, which does not match the
    values O29 publishes; whether the two protocols differ is a question for O29, and nothing here
  ...
  \subsection{The covariance rotation, and the two pairs where it matters}
    ...
    \paragraph{The column label.}
      Version~1.3 defined that column in its running text as the XY isotropy ratio
      $|\tilde L_{11} - \tilde L_{22}|/|\tilde L_{11}|$ on the covariance-rotated matrix, while its
      table caption called both printed columns ``relative cross-term ratios''.
      Evaluated on the stored-basis compression the expression is $0$, both entries being equal; on
      the reported matrix at $(151,5)$ it is $0.2772$, whereas the quantity actually printed is the
      cross-term ratio
