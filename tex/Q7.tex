...
\begin{document}
  ...
  \begin{abstract}
    ...
    scalar; choosing it real and positive additionally requires a phase convention.
    ...
    gives horizontal isotropy when compatible with the real rotation structure, but does not force
    $A_H = A_z$.
    ...
    be established.
    The broader bridge to an extended target remains a missing construction.
    The existing Q5b spatial principal symbol has rank two and cannot equal the rank-three
    Casimir form; the revised conjecture requires a new target.
    Separately, under Q5a's depth and weight hypotheses, no common scalar normalisation of its
    published admissibility form yields a toric differential limit.
  \end{abstract}
  ...
  \subsection{The Problem}
    ...
    isolated as [ACT] in Remark~\ref{rem:status}, it is unique up to one scalar by irreducibility
    (Lemma~\ref{lem:schur}); and it must
    satisfy a necessary form constraint (Criterion~\ref{crit:symbol}) on a newly supplied
    extended target.
    The precise identification with Q5b's rank-two spatial principal symbol is ruled out by rank
    preservation, whereas the broader extended-target construction remains missing.
    Separately, under Q5a's depth and weight hypotheses no common scalar normalisation of the
    published admissibility form produces a toric differential limit (Q5a Theorem~6.1).
  ...
  \subsection{The Admissible Three: The Neutral Sector of a Supplied Spinor Carrier}
    ...
    irreducible $\mathrm{SU}(2)$-valued two-dimensional representation
    $V_\rho \cong \mathbb{C}^2$ (a supplied input), then its traceless anti-Hermitian sector is
    $\su(2) \simeq \ImH$, carrying exactly
    three independent
  ...
  \subsection{Grading Correspondence}
    ...
    \begin{remark}[Grading correspondence]
      ...
      This is a proposed correspondence of subspaces, not a derivation of a differential symbol.
      In the Carnot grading $Z$ has degree two, whereas $Z^2$ has degree four.
      A commutator producing $Z$ does not by itself produce a quadratic term in $k_Z$.
      The correspondence is not a Lie algebra isomorphism (Proposition~\ref{prop:noniso}).
    \end{remark}
  ...
  \subsection{Rigidity of Any Bridge: Schur's Lemma}
    ...
    \begin{remark}[Consequence for the bridge]
      ...
      Once both isomorphic modules are supplied, non-zero intertwiners exist because their
      space has complex dimension one.
      The open question concerns the geometric origin of [ACT] and the additional compatibility
      with a spatial form, not the existence of an abstract intertwiner under these premises.
      ...
      one constrains nothing, since
      $\varphi^*h=|\lambda|^2\varphi_0^*h$ is positive definite for every $\lambda\neq0$.
      Equality with a specified form fixes the modulus only if that form is a positive scalar
      multiple of $\varphi_0^*h$; otherwise no scalar achieves equality.
      The phase remains free because it acts trivially on the pullback.
      Fixing the phase is a convention, to be stated wherever it is used.
      A unit scalar is available only when the chosen reference normalisation also fixes its
      modulus to one.
  ...
  \subsection{The Schr\"odinger Quadratic Form Lemma}
    ...
    \begin{lemma}[Schr\"{o}dinger quadratic form]
      ...
      \begin{enumerate}
        ...
        $-g(v,v)$.
        \item Assume additionally [ADAPT]: there is an equivariant isomorphism $\varphi_0$
        unitary for the chosen standard Hermitian form on the source and the Hermitian extension
        of the auxiliary real form $r_0=\mathrm{diag}(1,1,1)$ on the target, mapping the
        weight-$\pm1$ plane to the horizontal plane and the weight-$0$ line to the central line.
        This is a compatibility hypothesis on the action, grading and reference normalisation,
        not a consequence of [ACT] or Schur's lemma.
        Fix such an adapted isomorphism $\varphi_0$, which sends the weight vectors
    ...
    \begin{proof}
      ...
      The displayed diagonal form is an input to the lemma.
      A sufficient invariance contract, denoted [INV], is that the target action generated by
      $J_3$ is the complexification of the standard real rotations of the horizontal plane,
      fixes the central line, and preserves the real symmetric form $g_{\mathrm{sp}}$.
      Rotation invariance makes that real form scalar on the horizontal plane and eliminates
      its cross terms with the central line.
      Equivalently, its Hermitian extension is invariant and compatible with the real
      conjugation: $h_{\mathrm{sp}}(\sigma v,\sigma w)=\overline{h_{\mathrm{sp}}(v,w)}$.
      In a normalised weight basis this conjugation exchanges the two nonzero weights, so their
      diagonal coefficients agree.
      Hermitian $J_3$-invariance alone only makes the three weight spaces orthogonal:
      $\mathrm{diag}(a,b,c)$ is invariant for arbitrary positive $a,b,c$, and need not have $a=c$.
      Neither the real rotation argument nor [INV] forces $A_H=A_z$.
      No source establishes this action and real-form compatibility for the proposed spatial
      identification; it is an additional hypothesis here.
    ...
    \begin{remark}[Status of Lemma~\ref{lem:qform}]
      \label{rem:status}
      Item~(1) holds on the supplied spin-$1$ module without [ID]; [ID] is needed only to
      apply it to the measured $\Heff$.
      Item~(2) is the elementary preservation of positivity by an isomorphism, conditional on
      a supplied positive spatial form and on the target module [ACT].
      Abstract non-zero intertwiners then exist by Lemma~\ref{lem:schur}.
      Item~(3) additionally requires [ADAPT]; [INV], with its real-structure compatibility,
      explains the input two-coefficient form but does not establish [ADAPT].
      For example, in Cartesian coordinates of the real spin-$1$ module, let
      $S=\mathrm{diag}(1,1,2)$ and give the target the action
      $\rho_W=S\rho_{\mathrm{std}}S^{-1}$.
      It satisfies [ACT] and has the same horizontal rotations, hence satisfies [INV] for
      $g_{\mathrm{sp}}=\mathrm{diag}(A_H,A_H,A_z)$.
      Every intertwiner is $\lambda S$, whose pullback is
      $|\lambda|^2\mathrm{diag}(A_H,A_H,4A_z)$.
      No common scalar removes the relative factor four when both coefficients are positive.
      Thus [ADAPT] cannot be replaced by a choice of scalar under the earlier hypotheses.
      Q5b Theorem~5.2 supplies only a spatial principal form of rank two under [H-L] and
      [H-lift], with zero central slot in the adapted frame.
      It does not supply the positive rank-three form assumed here.
      Any such form is an additional construction, not a consequence of this lemma or an
      established import from Q5b; downstream asserted values are not used here.
    \end{remark}
  ...
  \subsection{The Bridge Conjecture}
    ...
    \begin{conjecture}[Bridge existence: extended target]
      \label{conj:bridge}
      A spatial construction beyond the rank-two principal form of Q5b supplies a positive
      rank-three form on $W_{\mathrm{sp}}$, an adjoint action [ACT], and the grading and real
      compatibility [INV] and [ADAPT].
      Under [ID], an equivariant identification then relates the measured space to this target;
      its pullback has the two-coefficient form of Lemma~\ref{lem:qform}(3).
      The map has the same type as in the earlier formulation,
      \begin{equation}
        \varphi:\Sym^2(\Vr)\xrightarrow{\sim}W_{\mathrm{sp}}\otimes_\mathbb{R}\mathbb{C},
      \end{equation}
      but its target form must now come from the new construction specified here.
      The construction must specify the operator or effective form producing this target and
      its relation to Q5b's horizontal geometry.
      This is a conjecture about supplying compatible geometric data, not about abstract
      intertwiners between already supplied isomorphic modules.
    \end{conjecture}

    \noindent
    The conjecture no longer asks for equality with Q5b's existing spatial principal symbol.
    Under [H-L] and [H-lift], that symbol is
    \[
      A_H\bigl[(k_x+\tfrac y2 k_z)^2+(k_y-\tfrac x2 k_z)^2\bigr].
    \]
    Its rank is two everywhere, with kernel generated by $(-y/2,x/2,1)$.
    Congruence by an isomorphism preserves rank, so it cannot equal the positive rank-three
    Casimir form $2\|\cdot\|^2$, even without equivariance.
    Adding terms of strictly lower ordinary differential order cannot change that principal
    symbol.
    The previous target contract is therefore ruled out under Q5b's stated hypotheses;
    the broader construction in Conjecture~\ref{conj:bridge} remains missing, not ruled out.
    Even for a new positive target, equality with the transported Casimir is a stronger condition
    than the graded form: under [ADAPT] it requires $A_H=A_z$ and a scale choice.
    Neither that equality nor an absolute coefficient is asserted by the revised conjecture.
    ...
    existence of a bridge to a different, explicitly constructed target.
    By Lemma~\ref{lem:schur}, the supplied adjoint modules admit intertwiners unique up to scale; by
  ...
  to one scalar.
  This does not establish the target geometry or the adapted normalisation [ADAPT].
  ...
  \begin{criterion}[Symbol restriction to $\Sym^2(\Vr)$]
    \label{crit:symbol}
    Let $L_{\mathrm{ext}}$ be a candidate continuum operator supplying the extended target of
    Conjecture~\ref{conj:bridge}, with spatial principal symbol on $(k_X,k_Y,k_Z)$.
    This is additional input, not the rank-two operator $L_{\mathrm{eff}}$ of Q5b.
    Under [ADAPT], use the source conjugation
    $\sigma_S=\varphi_0^{-1}\sigma\varphi_0$ and choose normalised weight phases so that
    $\sigma_S(e_+)=e_-$ and $\sigma_S(e_0)=e_0$.
    ...
    Test whether:
    \begin{equation}
      \sigma_2(L_{\mathrm{ext}})\big|_{\Sym^2(\Vr)} = A_H (k_X^2 + k_Y^2) + A_z k_Z^2,
      \quad A_H, A_z > 0.
    \end{equation}
    This is a necessary form test for the proposed extended continuum target, not a sufficient
    test of its geometric origin or equivariance.
    Applied instead to Q5b's existing operator, it fails by the rank argument above.
    No candidate extended operator is constructed or tested here;
  ...
  \begin{remark}[What is computable, and what Section~\ref{sec:numerical} computes]
    ...
    They do not evaluate $\sigma_2(L_{\mathrm{ext}})$, and the transfer of a discrete compression
  ...
  \subsection{Compressions of the discrete Weil Laplacian, and their closed form}
    ...
    which is the spectrum of no single pair there, and over the five pairs that hold projections at
    $q = 29$ it is $[1, 0.512, 0.512]$, which at that prime does coincide with one pair's spectrum.
    Neither rounds to either of the two figures at issue here, O29's $[1:0.6:0.6]$ and
    $[1:0.8:0.8]$.
    The $q = 29$ median does round to $[1:0.5:0.5]$, which is the figure O29's Figure~1 caption
    gives as a median normalised spectrum; that caption reports the leading three eigenvalues of the
    $9 \times 9$ covariance, a different object, and the coincidence is noted here only so that a
    reader who meets it is not left to suppose an agreement that is not claimed.
    ...
    $[1:0.6:0.6]$.
    O29 states no aggregation in the remark that gives these two figures, but it both pools and
    computes them.
    Its \texttt{variety\_characterization.py} concatenates the trajectory over all pairs at
    $q \in \{29, 61\}$ and prints the normalised singular values of the pooled $N \times 3$ matrix;
    those values are $[1, 0.747, 0.747]$ and $[1, 0.868, 0.867]$, whose squares are the two spectra
    above, because the eigenvalues of $\langle w w^\dagger \rangle$ are the squared singular values
    of that matrix.
    What this paper adds is only the identification of that quantity with the figures O29's remark
    publishes: O29 never assembles the $3 \times 3$ matrix, and the covariances it does assemble are
    those of vectorised outer products, on $\mathrm{End}(\Heff)$ and on
    $\mathrm{End}(U^{(2)}_c)$, the endomorphisms of a two-dimensional principal-subspace truncation
    of $\Heff$.
    ...
    \begin{corollary}[Status of the bridge identification]
      ...
      Hypothesis~[ID], and with the spatial sector carrying the adjoint action [ACT], an equivariant
      bridge is unique up to one scalar, and if it exists and the further premises of
      Remark~\ref{rem:status} hold, the spatial block is graded as in Lemma~\ref{lem:qform}.
    \end{corollary}

    \begin{remark}[Three-case structure of the isotropy question]
      \label{rem:threecases}
      Criterion~\ref{crit:symbol} tests a positive graded form, not the equality $A_H=A_z$.
      If a new continuum target and compatible identification have independently been supplied,
      its coefficients admit the following cases; none establishes the identification by itself.
      \begin{enumerate}
        \item $A_H=A_z>0$: the supplied form is isotropic in the adapted reference frame.
        Equality to a transported Casimir still requires its scale to be specified.
        \item $A_H\neq A_z$, both positive: the form is positive and graded but anisotropic.
        It meets the form test, not the stronger Casimir equality.
        \item $A_H\leq0$ or $A_z\leq0$: the form fails the positivity test for this realisation.
        This does not exclude another constructed target.
      \end{enumerate}
      ...
      These data exclude no case for a new target; Q5b's existing symbol has its known rank-two obstruction.
  ...
  \subsection{Reproducible diagnostic}
    ...
    So that a reader with the repository alone can re-run its default path, the inputs that path
    needs are shipped with
    ...
    $q\in\{61,101,151,211\}$, with filenames \texttt{q<q>\_o25.npz} of the Q5a-O5 campaign.
    ...
    The reader should note what this does and does not give.
    The bundle makes the per-pair statements of Sections~\ref{ssec:rayleigh}
    and~\ref{ssec:rotation} reproducible, those depending only on its arrays; the pooled and median
    figures and the pair counts are out of its reach, for the coverage reason given in
    Section~\ref{ssec:rayleigh}.
    It is in any case a derived extract, so reproducing the extraction itself requires the
    ...
    over the conjugate pairs that hold projections, which is the one matching the two figures at
    issue here, and the componentwise median of the per-pair spectra, which matches neither of them.
  ...
  \subsection{What This Paper Establishes}
    ...
    \begin{itemize}
      ...
      generator, together with the real compatibility [INV], gives horizontal isotropy;
      the coefficient transport additionally assumes [ADAPT] and does not force $A_H=A_z$.
      ...
      as a proposed matching of subspaces, without deriving a central differential term. The two-dimensional
      ...
      \item Every diagonal entry of the compression computed on the O25 checkpoints \emph{in the stored
    ...
    equivariant bridge together with the module structure on the measured space and the adjoint
    action on the spatial sector, is not supplied by the audited corpus, and no result proves that
    it cannot exist.

  \subsection{Relation to Open Problem Q5b-O2}

    Open problem Q5b-O2 concerns a possible full-rank spatial extension.
    The present lemma only describes the transport of such a form if it is supplied, together
    with [ACT], [INV] and [ADAPT]; it neither produces a central differential term nor fixes a
    coefficient.
    A term of lower ordinary differential order leaves Q5b's principal symbol unchanged.
    A proposed extension must therefore specify a new effective operator or another explicitly
    defined effective-form construction, rather than identify its positive form with the
    unchanged rank-two symbol.
  ...
  \subsection{If the Identification Succeeds}
    ...
    module of $\su(2)$.
    Such a construction would identify two supplied three-dimensional spaces; it would not
    independently select the dimension of physical space.
    Besides the supplied spinor carrier of O23~\cite{Beau2026a27} and O26's minimality selection,
    it would require [ID], [ACT], [INV], [ADAPT] and a construction of the extended spatial target.
    Q5b's [H-L] does not supply that full-rank extension.
  ...
  Under Hypothesis~[ID], which identifies the measured space with the spin-$1$ module, and with the
  spatial sector carrying the adjoint action [ACT], any equivariant bridge is either zero or an
  isomorphism unique up to one scalar; and, if that bridge is non-zero, the Hermitian spatial form
  pulls back to a graded form on the module, isotropic within the horizontal plane under the
  real rotation compatibility [INV] and the adapted normalisation [ADAPT], with two constants
  $A_H$ and $A_z$, neither evaluated here.
  None of [ID], [ACT], [INV] and [ADAPT] is established for this geometric construction.
  ...
  basis rows, and those rows are pure Fourier modes, so every diagonal entry is the Rayleigh quotient
  ...
  The exact rank-two principal symbol of Q5b is incompatible with a rank-three positive form;
  the revised conjecture leaves open a different, explicitly constructed target.
  A separate conditional obstruction applies under the depth and
  ...
  Conjecture~\ref{conj:isotropy} testable; and a construction of an extended spatial target with
  a specified operator and compatible action, to which Criterion~\ref{crit:symbol} could apply.
